import {savedAt,savedStops} from './nearby-stops.js';
...
export async function nearbyResponse(request){
 ...
 const saved=normalizeStops(savedStops,lat,lon);
 const snapshot=()=>respond({stops:saved,updatedAt:savedAt,source:'OpenStreetMap',snapshot:true});
 if(input.preset==='dongtan'&&lat===37.2&&lon===127.0955)return snapshot();
 try{
 ...
 }catch{if(saved.length)return snapshot();return respond({error:'unavailable',message:'주변 정류소 조회가 지연되고 있어요. 다시 시도하거나 아래 지도 검색을 이용하세요.'},502);}
}
